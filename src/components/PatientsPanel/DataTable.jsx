...
function AddNewItem({ config }) {
  ...
  return (
    <Button onClick={handelClick} type="primary" icon={<PlusOutlined />} style={{ backgroundColor:'rgb(255, 160, 176)',color:'white',borderColor:'rgb(255, 160, 176)' }}>
      {ADD_NEW_ENTITY}
    </Button>
  );
}
...
export default function DataTable({ config, DataTableDropMenu }) {
  ...
  const handelDataTableLoad = useCallback((pagination, filters, sorter) => {
    const options = { page: pagination.current || 1, filters, sorter,arrFiltersReq };
    dispatch(erp.list({ listURL, options }));
  }, []);

  useEffect(() => {
    dispatch(erp.list({ listURL,options:{arrFiltersReq,filters:filters||[],sorter:sorter||[]} }));
  }, []);
